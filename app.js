...
app.get("/home", function (req, res) {
  ...
  if (sess.uid) {
    con.query("set GLOBAL event_scheduler = ON ", function (err, rows_03) {
      ...
      con.query(
        "select * from users where id = '" + sess.uid + "' ",
        function (err, rows_01) {
          con.query(
            "select company_name,logo from company_info ",
            function (err, rows_02) {
              ...
              con.query("select * from users  ", function (err, rows_03) {
                res.render("home", {
                  data_02: rows_02,
                  data_03: rows_03,
                  user_name: rows_01[0].full_name,
                  profile: rows_01[0].profile,
                  us_id: rows_01[0].id,
                  authority: rows_01[0].authority,
                });
              });
              /*  }else{
...
app.post("/add_users", function (req, res) {
  if (!(req.files && req.files.image)) {
    ...
    var update_query1 =
      "INSERT INTO `users`(`username`, `full_name`, `authority`, `password`,`profile`) VALUES ('" +
      user_name +
      "','" +
      full_name +
      "','" +
...
app.get("/update_sales.ejs", function (req, res) {
  con.query("select * from company_info ", function (err, rows_02) {
    ...
    con.query(
      "SELECT bill_items.*, bill_details.bill_id FROM bill_items INNER JOIN bill_details ON bill_items.bill_detail_id = bill_details.id WHERE bill_details.bill_id = '" +
      ...
      function (err, rows_05) {
        ...
        con.query("select * from stack_to_market ", function (err, rows_03) {
          con.query(
            "SELECT froshat_details.email,froshat_details.bill_no,froshat_details.contact,froshat_details.total_amount,froshat_details.paid_amount, customer_account.name FROM froshat_details INNER JOIN customer_account ON froshat_details.cus_id = customer_account.id WHERE froshat_details.id = '" +
              req.query.customer_id +
              "' ",
...
app.get("/dashboard.ejs", function (req, res) {
  con.beginTransaction(function (err) {
    ...
    con.query(
      "SELECT round(COALESCE(SUM(price * quantity/ex_rate),0),3)AS total FROM expenses",
      function (error, results, fields) {
        ...
        con.query(
          "SELECT round(SUM(total_amount/ex_rate),3)AS sell_total FROM froshat_details",
          function (error, results1, fields) {
            ...
            con.query(
              "SELECT round(SUM((quantity* price)/ex_rate),3)AS sell_total4 FROM stack_raw_materials",
              function (error, rows_purchase, fields) {
                if (error) {
                  return con.rollback(function () {
                    throw error;
                  });
                ...
                con.query(
                  "SELECT round(SUM(amount/ex_rate),3)AS company_loan FROM incoming_loan",
                  function (error, results2, fields) {
                    ...
                    con.query(
                      "SELECT round(SUM(amount/ex_rate),3)AS company_loan FROM outgoing_loan",
                      function (error, results3, fields) {
                        ...
                        con.commit(function (err) {
                          ...
                          res.render("dashboard", {
                            expenses: results[0].total,
                            purchase: rows_purchase[0].sell_total4,
                            sell_amount: results1[0].sell_total,
                            incoming_loan: results2[0].company_loan,
...
app.get("/customer_view.ejs", function (req, res) {
  ...
  con.query("SELECT * from customer_account", function (err, rows_03) {
    con.query(
      "SELECT customer_account.id as cust_id, customer_account.company_name, froshat_details.* FROM customer_account INNER JOIN froshat_details ON customer_account.id=froshat_details.cus_id WHERE  customer_account.id ='" +
        my_id +
        "'",
      function (err, rows_01) {
        if (rows_01.length > 0) {
          ...
          for (var i = 0; i < rows_01.length; i++) {
            var sh = moment(rows_01[i].date, "YYYY/MM/DD")
              .locale("fa")
              .format("YYYY/MM/DD");
            arr += sh + ",";
          }
...
app.post("/stack_2_details", function (req, res) {
  ...
  con.beginTransaction(function (err) {
    if (err) {
      ...
      con.query(
        "select bill_no from stack_to_market_details where bill_no = '" +
          bill_no +
          "'",
        function (err, rows_001) {
          if (rows_001.length > 0) {
            res.send("معذرت! بل قبلا ثبت شده");
          } else {
            con.query(
              "INSERT INTO stack_to_market_details(`stuff_id`, `bill_no`,`date`) VALUES ('" +
                stf_id +
                "','" +
                bill_no +
                "','" +
              ...
              function (error, results, fields) {
                ...
                for (var i = 0; i < items_array.length - 1; i++) {
                  (function (i) {
                    setTimeout(function () {
                      con.query(
                        "insert into stack_bill_detail(stack_to_m_detail_id,item_name,category,quantity,price) values('" +
                          lst_id +
                          "','" +
                          items_array[i] +
                          "','" +
                      ...
                      con.query(
                        "insert into stack_to_market(item_name,item_type,fixed_price,quantity,date) values('" +
                          items_array[i] +
                          "','" +
                          buy_type_array[i] +
                          "','" +
                                  ...
                                  con.query(
                                    ...
                                    function (err, rows_006) {
                                      if (rows_006.length > 0) {
                                        ...
                                        var update_quna_01 =
                                          parseFloat(db_quantity_01) +
                                          parseFloat(web_quantity_01);
                                        con.query(
                                        ...
                                        con.query(
                                          "insert into stack_to_market_lists (item_name,item_type,sell_price,quantity) values('" +
                                            items_array[i] +
                                            "','" +
                                            buy_type_array[i] +
                                            "','" +
...
app.post("/froshat_details", function (req, res) {
  ...
  if (typeof stak_to_market_id_raw === "string") {
    stak_to_market_id = stak_to_market_id_raw
      .split(",")
      .filter((item) => item && item.trim() !== "");
  ...
  var new_thickness =
    (thickness.length > 0 ? thickness.join(",") : "0,".repeat(items.length)) +
    newstr;

  var items_array = new_items
    .split(",")
    .filter((item) => item && item.trim() !== "someerr");
  var buy_type_array = new_type
    .split(",")
    .filter((item) => item && item.trim() !== "someerr");
  var quantity_array = new_quantity
    .split(",")
    .filter((item) => item && item.trim() !== "someerr");
  var price_array = new_price
    .split(",")
    .filter((item) => item && item.trim() !== "someerr");
  ...
  con.beginTransaction(function (err) {
    if (err) {
      ...
      con.query(
        "select bill_no from froshat_details where bill_no = '" + bill_no + "'",
        function (err, rows_001) {
          if (err) {
            return con.rollback(function () {
          ...
          if (rows_001.length > 0) {
            ...
            con.query(
              "INSERT INTO froshat_details(`cus_id`,`contact`,`bill_no`,`total_amount`,`paid_amount`,`currency`,`ex_rate`,`date`) VALUES ('" +
                customer_name +
                "','" +
                customer_contact +
                "','" +
              ...
              function (error, results, fields) {
                if (error) {
                  return con.rollback(function () {
                    res
                      .status(500)
                      .send("خطا در ثبت اطلاعات: " + error.message);
                  });
                }
                var lst_id = results.insertId;

                con.query(
                  "insert into sales_payments(`sales_id`, `paid`, `currency`, `ex_rate`, `date`) values('" +
                    lst_id +
                    "','" +
                    received_show +
                    "','" +
                    ...
                    m +
                    "')",
                  function (err, rows_009) {
                    if (err) {
                      return con.rollback(function () {
                        res
                          .status(500)
                          .send("خطا در ثبت پرداخت: " + err.message);
                      });
                    }
                    ...
                    if (received_show > 0) {
                      con.query(
                        "insert into bill_payment(`froshat_details_id`, `paid_amount`, `currency`, `ex_rate`, `payment_date`, `description`) values('" +
                          lst_id +
                          "','" +
                          received_show +
                          "','" +
                    ...
                    for (var i = 0; i < items_array.length; i++) {
                      (function (i) {
                        setTimeout(function () {
                          ...
                          con.query(
                            "insert into bill_details(bill_id,item_name,item_type,quantity,price) values('" +
                              lst_id +
                              "','" +
                              items_array[i] +
                              "','" +
                      ...
                      con.commit(function (err) {
                        if (err) {
                          return con.rollback(function () {
                            res
                              .status(500)
                              .send("خطا در ثبت نهایی: " + err.message);
                          });
                        }
...
app.post("/update_froshat_details", function (req, res) {
  ...
  con.beginTransaction(function (err) {
    if (err) {
      ...
      con.query(
        "select total_amount from froshat_details where id='" +
          custoemr_id +
          "' ",
        function (error, results, fields) {
          if (error) {
          ...
          con.query(
            "update froshat_details set `customer_name`='" +
              customer_name +
              "', `email`='" +
              customer_email +
              "',`contact`='" +
            ...
            function (error, results, fields) {
              ...
              for (var i = 0; i < items_array.length - 1; i++) {
                (function (i) {
                  setTimeout(function () {
                    con.query(
                      "insert into return_bill(bill_id,item_name,item_type,quantity,price) values('" +
                        custoemr_id +
                        "','" +
                        items_array[i] +
                        "','" +
                    ...
                    con.query(
                      "select * from return_bill where bill_id = '" +
                        custoemr_id +
                        "'",
                      function (error, results2, fields) {
                        if (error) {
                        ...
                        con.query(
                          "select * from stack_to_market where item_name='" +
                            ...
                            buy_type_array[i] +
                            "'",
                          function (error, results6, fields) {
                            if (error) {
                              return con.rollback(function () {
                                throw error;
                              });
                            ...
                            update_quantity =
                              parseInt(db_quantity) +
                              parseInt(quantity_array[i]);
                            console.log(update_quantity);
...
app.post("/baraword_mashol", function (req, res) {
  ...
  con.query(
    "select * from set_price where name='" +
      mahsole_name +
      "' and type='" +
      mahsole_type +
      "'",
    function (err, rows_05) {
      if (rows_05.length > 0) {
        ...
        con.query(
          "INSERT INTO `set_price`(`name`, `type`,`ex_rate`,`total_price`) VALUES ('" +
            mahsole_name +
            "','" +
            mahsole_type +
            "','" +
          ...
          function (err, rows_0031) {
            ...
            var exp_name = "مصارف";
            con.query(
              "INSERT INTO `mahsol_price`(`set_p_id`, `item_name`,`quantity`, `price`) VALUES ('" +
                item_type_id +
                "','" +
                exp_name +
                "','1','" +
              ...
              function (err, rows_0031) {
                for (var i = 0; i < items_array.length - 1; i++) {
                  (function (i) {
                    setTimeout(function () {
                      con.query(
                        "INSERT INTO `mahsol_price`( `set_p_id`, `item_name`, `item_type`, `quantity`, `price`) VALUES ('" +
                          item_type_id +
                          "','" +
                          items_array[i] +
                          "','" +
    ...
    function (err, rows_0031) {
      ...
      for (var i = 0; i < items_array.length - 1; i++) {
        (function (i) {
          setTimeout(function () {
            con.query(
              "INSERT INTO `create_mahsol`( `ready_material_type_id`, `item_name`, `item_type`, `quantity`, `price`) VALUES ('" +
                item_type_id +
                "','" +
                items_array[i] +
                "','" +
              ...
              function (err, rows_03) {
                con.query(
                  "select * from stack_raw_materials_lists where item_name ='" +
                    items_array[i] +
                    "' and item_type='" +
                    type_array[i] +
                    "'",
                  function (err, rows_01) {
                    ...
                    con.query(
                      "update stack_raw_materials_lists set quantity='" +
                        update_quantity +
                        "' where item_name ='" +
                        items_array[i] +
                        "' and item_type='" +
...
app.get("/company-reg.ejs", function (req, res) {
  con.query("select * from company_info ", function (err, rows_02) {
    ...
    for (var i = 0; i < rows_02.length; i++) {
      var sh = moment(rows_02[i].starting_activity, "YYYY/MM/DD")
        .locale("fa")
        .format("YYYY/MM/DD");
      arr += sh + ",";
    }
...
app.get("/cs-reg.ejs", function (req, res) {
  con.query(
    "SELECT *,(SELECT round(COALESCE(SUM(total_amount ),0),3)AS total FROM froshat_details WHERE cus_id=customer_account.id) AS total_amount ,(SELECT round(COALESCE(SUM(paid_amount),0),3)AS total_paid FROM froshat_details WHERE cus_id=customer_account.id) AS cus_paid FROM customer_account ",
    function (err, rows_01) {
      if (rows_01.length > 0) {
        con.query(
          "SELECT COUNT(id) as total_customers from  `customer_account` ",
          function (err, rows_02) {
            /* var arr = [];
                                                for(var i =0 ; i<rows_01.length;i++)
                                                {
...
app.get("/dis-reg.ejs", function (req, res) {
  con.query(
    "SELECT round(SUM((price * quantity)/ex_rate),3) AS total FROM expenses",
    function (err, rows_04) {
      con.query(
        "SELECT expenses.*, expense_category.name FROM expenses INNER JOIN expense_category ON expenses.category_id = expense_category.id",
        function (err, rows_03) {
          if (rows_03.length > 0) {
            var arr = [];
            for (var i = 0; i < rows_03.length; i++) {
              var sh = moment(rows_03[i].date, "YYYY/MM/DD")
                .locale("fa")
                .format("YYYY/MM/DD");
              arr += sh + ",";
            }
...
app.post("/get_all_types", function (req, res) {
  ...
  con.query(
    "SELECT * FROM material_type where id = '" + my_id + "'",
    function (err, rows_02) {
...
app.post("/get_all_types_3", function (req, res) {
  ...
  con.query(
    "SELECT * FROM customer_account where id = '" + my_id + "'",
    function (err, rows_02) {
...
app.post("/get_stuff_salary", function (req, res) {
  ...
  con.query(
    "SELECT *,(SELECT SUM(amount)AS total FROM taken_amount WHERE stuff_id=stuff_registration.id) AS totalcount FROM stuff_registration WHERE stuff_registration.id = '" +
      my_id +
      "'",
    function (err, rows_02) {
      res.send(rows_02);
...
app.post("/updated_froshats", function (req, res) {
  ...
  con.query(
    "UPDATE `froshat_details` SET cus_id='" +
      comp_name +
      "',`currency`='" +
      currency +
      "',`ex_rate`='" +
...
app.post("/raw_reg_delete", function (req, res) {
  ...
  con.query(
    "select * from stack_factory_registration WHERE id = '" + my_id + "'",
    function (err, rows_01) {
      ...
      con.query(
        ...
        function (err, rows_03) {
          ...
          con.query(
            ...
            function (err, rows_04) {
              con.query(
                "delete from stack_factory_registration WHERE id = '" +
                  my_id +
                  "'",
                function (err, rows_02) {
                  con.query(
                    "delete from raw_material_each_mahsol WHERE stack_factory_id = '" +
                      my_id +
                      "'",
...
app.post("/delete_stuff_paid", function (req, res) {
  ...
  con.query(
    "select * from payable_amount WHERE id = '" + my_id + "'",
    function (err, rows_01) {
      var paid_amount = rows_01[0].taken_amount;
      var stuff_id = rows_01[0].stuff_id;
      con.query(
        "select * from stuff_registration WHERE id = '" + stuff_id + "'",
        function (err, rows_02) {
          con.query(
            "select * from taken_amount WHERE id = '" + stuff_id + "'",
            function (err, rows_03) {
              ...
              con.query(
                "update taken_amount set amount='" +
                  update_taken_amount +
                  "' WHERE id = '" +
                  take_id +
                  "'",
...
app.post("/add_payment_delete", function (req, res) {
  ...
  con.query(
    "delete from incoming_loan_list WHERE id = '" + my_id + "'",
    function (err, rows_02) {
      if (err) {
        throw err;
      } else {
        con.query(
          "select * from incoming_loan_list where incoming_loan_id ='" +
            loan_id +
            "'",
          function (err1, rows_04) {
            if (err1) {
              throw err1;
            } else {
              var table_data = "";
              ...
              rows_04.forEach((row) => {
                ...
                table_data +=
                  ...
                  row.id +
                  ")" +
                  " style='color:red;' href='#'>حذف /</a>     <a href='#' style='color:green;' onclick=" +
                  "payment_edit(" +
                  row.id +
                  ")" +
                  ">ویرایش</a></td>";
...
app.post("/add_payment_delete_001", function (req, res) {
  ...
  con.query(
    "delete from outgoing_loan_list WHERE id = '" + my_id + "'",
    function (err, rows_02) {
      if (err) {
        throw err;
      } else {
...
app.post("/froshat_view_delete", function (req, res) {
  ...
  con.query(
    "Select bill_id from bill_details where id = '" + bill_id + "' limit 1",
    function (err, rows_01) {
      if (err) {
        res.json({ message: "خطا در دریافت اطلاعات!" });
        return;
      }
      if (!rows_01 || rows_01.length === 0) {
      ...
      con.query(
        ...
        function (err, rows_02) {
          if (err) {
            res.json({ message: "این شخص معامله دارد نمیتوانید حذف کنید! " });
          } else {
...
app.post("/delete_parent_mahsol_formula", function (req, res) {
  ...
  con.query(
    "delete from create_mahsol WHERE ready_material_type_id= '" + my_id + "'",
    function (err, rows_02) {
      con.query(
...
app.post("/delete_price_mahsol_formula", function (req, res) {
  ...
  con.query(
    "delete from mahsol_price WHERE set_p_id= '" + my_id + "'",
    function (err, rows_02) {
      con.query(
...
app.post("/update_expense", function (req, res) {
  ...
  var query2 =
    "SELECT expenses.*, expense_category.name FROM expenses INNER JOIN expense_category ON expenses.category_id = expense_category.id  WHERE expenses.id = '" +
    my_id +
    "'";
  con.query(query2, function (err, rows_02) {
    var sh = moment(rows_02[0].date, "YYYY/MM/DD")
      .locale("fa")
      .format("YYYY/MM/DD");
...
app.post("/graph_data", function (req, res) {
  ...
  con.query(
    "SELECT round(COALESCE(SUM(total_amount/ex_rate),0),3) as total_sell FROM `froshat_details` WHERE date BETWEEN '" +
      all_date[0] +
      "' AND '" +
      all_date[1] +
      "' ",
                                            ...
                                            function (err, rows_11) {
                                              con.query(
                                                "SELECT round(COALESCE(SUM(total_amount/ex_rate),0),3) as total_sell FROM `froshat_details` WHERE date BETWEEN '" +
                                                  ...
                                                  all_date[23] +
                                                  "' ",
                                                function (err, rows_12) {
                                                  var date_only1 =
                                                    rows_01[0].total_sell;
                                                  var date_only2 =
                                                    rows_02[0].total_sell;
                                                  var date_only3 =
                                                    rows_03[0].total_sell;
                                                  var date_only4 =
                                                    rows_04[0].total_sell;
                                                  var date_only5 =
                                                    rows_05[0].total_sell;
                                                  var date_only6 =
                                                    rows_06[0].total_sell;
                                                  var date_only7 =
                                                    rows_07[0].total_sell;
                                                  var date_only8 =
                                                    rows_08[0].total_sell;
                                                  var date_only9 =
                                                    rows_09[0].total_sell;
                                                  var date_only10 =
                                                    rows_10[0].total_sell;
                                                  var date_only11 =
                                                    rows_11[0].total_sell;
                                                  var date_only12 =
                                                    rows_12[0].total_sell;
          ...
          if (err1) {
            ...
            rows_04.forEach((row) => {
              ...
              table_data +=
                ...
                row.id +
                ")" +
                " href=# style='color:red;'>حذف /</a>     <a onclick=" +
                "cat_edit(" +
                row.id +
                ")" +
                " href=# style='color:green;'>ویرایش</a></td>";
              table_data += "</tr>";
              no++;
            });
            res.send(table_data);
...
app.post("/update_raw_material_01", function (req, res) {
  ...
  var update_query =
    "UPDATE `stack_raw_materials` SET `buy_type`='" +
    buy_type +
    "',`company_name`='" +
    com_name +
    "',`contact`='" +
...
app.post("/update_rmf_material_01", function (req, res) {
  ...
  con.query(
    ...
    function (err, rows_3) {
      ...
      con.query(
        "select * from stack_factory_registration where id='" + edit_id + "' ",
        function (err, rows_4) {
          ...
          con.query(
            ...
            function (err, rows_5) {
              if (err) {
                ...
                con.query(
                  "UPDATE `stack_factory_registration` SET `item_name`='" +
                    item_name +
                    "',`item_type`='" +
                    item_type +
                    "',`quantity`='" +
...
app.post("/update_emp_reg_01", function (req, res) {
  if (!(req.files && req.files.image)) {
    ...
    /* if (!(req.files))
                            { */
    var update_query1 =
      "UPDATE `stuff_registration` SET `name`='" +
      emp_name +
      "',`last_name`='" +
      last_name +
      "',`contact`='" +
...
app.post("/update_emp_taken", function (req, res) {
  ...
  var query2 =
    "SELECT taken_amount.id as tken_id,taken_amount.amount,taken_amount.currency,taken_amount.ex_rate,taken_amount.date , stuff_registration.* from taken_amount INNER JOIN stuff_registration ON taken_amount.stuff_id = stuff_registration.id WHERE taken_amount.id = '" +
    my_id +
    "'";
  con.query(query2, function (err, rows_02) {
    var sh = moment(rows_02[0].date, "YYYY/MM/DD")
...
app.post("/update_emp_salry", function (req, res) {
  ...
  var update_query =
    "UPDATE `payable_amount` SET `taken_amount`='" +
    payable_amount +
    "',`tax`='" +
    tax +
    "',`overtime`='" +
...
app.post("/update_inloan_reg", function (req, res) {
  ...
  var update_query =
    "UPDATE `incoming_loan` SET `borrower`='" +
    borrower +
    "',`lender`='" +
    lender +
    "',`amount`='" +
...
app.post("/update_o_nloan_reg", function (req, res) {
  ...
  var update_query =
    "UPDATE `outgoing_loan` SET `company_name`='" +
    company_name +
    "',`amount`='" +
    out_amount +
    "',`installment_no`='" +
...
app.post("/update_machine_tools", function (req, res) {
  ...
  var query2 =
    "SELECT * FROM  item_registration where machine_life IS NULL and id = '" +
    my_id +
    "'";
  con.query(query2, function (err, rows_02) {
    console.log(query2);
...
app.post("/udpate_machine_tools_02", function (req, res) {
  ...
  var update_query =
    "UPDATE `item_registration` SET`name`='" +
    name +
    "',`company_name`='" +
    company_name +
    "',`description`='" +
...
app.post("/update_customer_02", function (req, res) {
  ...
  var update_query =
    "UPDATE `customer_account` SET `name`='" +
    name +
    "',`last_name`='" +
    last_name +
    "',`company_name`='" +
...
app.post("/update_partners_02", function (req, res) {
  ...
  if (!(req.files && req.files.image)) {
    var update_query1 =
      "UPDATE `partner_registration` SET `full_name`='" +
      full_name +
      "',`location`='" +
      location +
      "',`contact`='" +
...
app.post("/update_hb_reg_02", function (req, res) {
  if (!(req.files && req.files.image)) {
    ...
    var update_query =
      "UPDATE `partner_taken_amount` SET `partner_id`='" +
      partner_id +
      "',`amount`='" +
      taken_amounts +
      "',`date`='" +
...
app.post("/update_comapny_info", function (req, res) {
  ...
  con.query(query2, function (err, rows_02) {
    console.log(query2);
    var sh = moment(rows_02[0].starting_activity, "YYYY/MM/DD")
      .locale("fa")
      .format("YYYY/MM/DD");
...
app.post("/update_com_info_02", function (req, res) {
  if (!(req.files && req.files.image)) {
    ...
    var update_query =
      ...
      m_date +
      "',`backup_url`='" +
      mysql_real_escape_string(back_up) +
      "',`currency`='" +
      currency +
      "' WHERE id = '" +
      edit_id +
...
app.post("/update_froshat_view", function (req, res) {
  ...
  var query2 =
    "SELECT customer_account.company_name, froshat_details.*  from customer_account,froshat_details where customer_account.id=froshat_details.cus_id AND  froshat_details.id = '" +
    my_id +
    "'";
  con.query(query2, function (err, rows_02) {
    console.log(query2);
    ...
    function (err, rows_04) {
      ...
      if (rows_04.length < 1) {
        ...
        for (var i = 0; i < rows_04.length; i++) {
          var sh = moment(rows_04[i].date, "YYYY/MM/DD")
            .locale("fa")
            .format("YYYY/MM/DD");
          shamsi += sh + ",";
        }
        ...
        if (err) {
          ...
          rows_04.forEach((row) => {
            ...
            table_data +=
              ...
              row.id +
              ")" +
              " href='#' style='color:red;'>حذف /</a>     <a href='#' style='color:green;' onclick=" +
              "payment_edit(" +
              row.id +
              ")" +
              ">ویرایش</a></td>";
...
app.post("/add_input_loan", function (req, res) {
  ...
  con.query(
    "INSERT INTO `incoming_loan_list`(`incoming_loan_id`, `paid`, `currency`, `ex_rate`, `date`) VALUES ('" +
      incoming_loan_id +
      "','" +
      amount +
      "','" +
...
app.post("/searching_bill_no", function (req, res) {
  ...
  var result = [];
  var f_d = moment
    .from(from_date, "fa", "YYYY/MM/DD")
    .locale("en")
    .format("YYYY/MM/DD");
  var t_d = moment
    .from(to_date, "fa", "YYYY/MM/DD")
    .locale("en")
    .format("YYYY/MM/DD");
  ...
  con.query(
    "SELECT customer_account.company_name, froshat_details.* FROM customer_account INNER JOIN froshat_details ON customer_account.id=froshat_details.cus_id WHERE froshat_details.date BETWEEN '" +
      f_d +
      "' AND '" +
      t_d +
      "' ORDER BY froshat_details.date DESC, froshat_details.id DESC",
    function (err, rows_02) {
      if (err) {
      ...
      rows_02.forEach((row) => {
        ...
        var formatted_date = moment(row.date, "YYYY/MM/DD")
          .locale("fa")
          .format("YYYY/MM/DD");
        ...
        table_data +=
          ...
          (row.company_name || "نامشخص") +
          "</td>";
        table_data +=
          '<td style="width:100px;">' + (row.contact || "-") + "</td>";
        ...
        table_data +=
          '<button data-toggle="tooltip" title="حذف" data-bill-id="' +
          row.id +
          '" class="edt del btn-delete"><img width="15px" src="assets/img/last-project/delete.svg" alt=""></button>';
        table_data +=
          '<button title="ویرایش" data-toggle="tooltip" data-bill-id="' +
          row.id +
          '" class="edt edit-tbl btn-edit"><img width="15px" src="assets/img/last-project/edit.svg" alt=""></button>';
        table_data +=
          '<button data-toggle="tooltip" title="اضافه کردن" data-bill-id="' +
          row.id +
          '" class="edt add-tbl btn-add-payment"><img width="15px" src="assets/img/last-project/add.svg" alt=""></button>';
        table_data +=
          '<button data-toggle="tooltip" title="چاپ بل" data-bill-id="' +
          row.id +
          '" class="edt print-tbl btn-print" style="background: #17a2b8; border: none; padding: 5px; border-radius: 3px;"><img width="15px" src="assets/img/mawadkham/print.svg" alt=""></button>';
        table_data += "</td>";
        table_data += "</tr>";
        table_data += "</table>";
        table_data += '<h2 class="mb-0">';
        table_data +=
          '<button type="button" class="btn btn-link" data-toggle="collapse" data-target="#collapseOne_' +
          row.id +
          '" style="float:left;"><img src="assets/img/expand_arrow_26px.png" data-bill-id="' +
          row.id +
          '" id="' +
          row.id +
          '" alt="" class="btn-toggle-details"></button>';
        table_data += "</h2>";
        table_data += "</div>";
        table_data +=
          '<div id="collapseOne_' +
          ...
          con.query(
            ...
            function (err, rows_06) {
              if (err) {
                ...
                con.query(
                  "SELECT customer_account.company_name, froshat_details.* FROM customer_account INNER JOIN froshat_details ON customer_account.id=froshat_details.cus_id WHERE  froshat_details.id ='" +
                    forsh_id +
                    "'",
                  function (err1, rows_004) {
                    ...
                    con.query(
                      "select * from sales_payments where sales_id ='" +
                        forsh_id +
                        "'",
                      function (err1, rows_04) {
                        var shamsi = [];
...
app.post("/add_output_sales", function (req, res) {
  ...
  con.query(
    ...
    function (err, rows_02) {
      ...
      con.query(
        "select * from bill_payment where froshat_details_id ='" +
          forsh_id +
        ...
        function (err1, rows_04) {
          if (err1) {
            return res
              .status(500)
              .send("خطا در دریافت لیست پرداخت ها: " + err1.message);
          }

          var shamsi = [];
          for (var i = 0; i < rows_04.length; i++) {
            var sh = moment(rows_04[i].payment_date, "YYYY-MM-DD")
              .locale("fa")
              .format("YYYY/MM/DD");
            shamsi += sh + ",";
          }
          ...
          rows_04.forEach((row) => {
            ...
            table_data +=
              ...
              row.id +
              ")" +
              " class='btn btn-sm btn-info' style='margin:2px;'>ویرایش</button> <button onclick=" +
              "payment_delet(" +
              row.id +
              ")" +
              " class='btn btn-sm btn-danger' style='margin:2px;'>حذف</button></td>";
            table_data += "</tr>";
            no++;
            no1++;
          });
...
app.post("/show_all_payment_001", function (req, res) {
  var each_loaner = req.query.each_loaner_id;
  con.query(
    "select * from outgoing_loan_list where outgoing_loan_id ='" +
      each_loaner +
      "'",
    function (err, rows_04) {
      var shamsi = [];
...
app.post("/add_output_loan", function (req, res) {
  ...
  con.query(
    "INSERT INTO `outgoing_loan_list`(`outgoing_loan_id`, `paid`, `currency`, `ex_rate`, `date`) VALUES ('" +
      outgoing_loan_id +
      "','" +
      amount +
      "','" +
...
app.post("/update_show_edit_001", function (req, res) {
  ...
  var m = moment
    .from(date_man, "fa", "YYYY/MM/DD")
    .locale("en")
    .format("YYYY/MM/DD");
  var loan_id = req.query.loan_id;
  con.query(
    "UPDATE `outgoing_loan_list` SET `outgoing_loan_id`='" +
      loan_id +
      "',`paid`='" +
      amount +
      "',`currency`='" +
...
app.post("/show_all_payment_003", function (req, res) {
  ...
  con.query(
    "select * from sales_payments where sales_id ='" + each_loaner + "'",
    function (err, rows_04) {
      var shamsi = [];
...
app.post("/update_bills_01", function (req, res) {
  ...
  con.query(
    ...
    function (err, rows_08) {
      ...
      con.query(
        "SELECT bill_items.*, bill_details.bill_id FROM bill_items INNER JOIN bill_details ON bill_items.bill_detail_id = bill_details.id WHERE bill_items.id ='" +
          bil_id +
          "' ",
        function (err, rows_07) {
          ...
          con.query(
            ...
            function (err, rows_02) {
              ...
              con.query(
                "UPDATE `bill_items` SET `quantity`='" +
                  quantity +
                  "',`price`='" +
                  price +
                  "',`thickness`='" +
                ...
                function (err, rows_02) {
                  if (err) {
                    return res
                      .status(500)
                      .send("خطا در بروزرسانی آیتم: " + err.message);
                  }
...
app.post("/update_stack_bills_01", function (req, res) {
  ...
  con.query(
    ...
    function (err, rows_08) {
      var stk_qunt = rows_08[0].quantity;
      con.query(
        "select * from stack_bill_detail where id ='" + bil_id + "' ",
        function (err, rows_07) {
          var db_quantity = rows_07[0].quantity;
          ...
          con.query(
            ...
            function (err, rows_09) {
              ...
              if (db_quantity > quantity) {
                //this case is ok,stk_qunt_list =2 , web =3
                update_qun = db_quantity - quantity;
                var sta_update_quan = stk_qunt + update_qun;
              ...
              con.query(
                ...
                function (err, rows_02) {
                  con.query(
                    "UPDATE `stack_bill_detail` SET `quantity`='" +
                      quantity +
                      "' where id = '" +
                      bil_id +
                      "' ",
...
app.post("/update_formula_01", function (req, res) {
  ...
  con.query(
    "UPDATE `create_mahsol` SET `quantity`='" +
      quantity +
      "',item_name='" +
      item_name +
      "' , item_type='" +
...
app.post("/sales_payment_edit_001", function (req, res) {
  ...
  con.query(query2, function (err, rows_02) {
    ...
    var sh = moment(rows_02[0].payment_date, "YYYY-MM-DD")
      .locale("fa")
      .format("YYYY/MM/DD");
...
app.post("/update_show_edit_002", function (req, res) {
  ...
  var description = req.query.description || "پرداخت";
  con.query(
    "UPDATE `bill_payment` SET `froshat_details_id`='" +
      loan_id +
      "',`paid_amount`='" +
      amount +
      "',`currency`='" +
...
app.post("/add_payment_delete_002", function (req, res) {
  ...
  con.query(
    "delete from bill_payment WHERE id = '" + my_id + "'",
    function (err, rows_02) {
      if (err) {
      ...
      con.query(
        ...
        function (err1, rows_04) {
          ...
          if (rows_04.length === 0) {
            table_data =
              "<tr><td colspan='7' style='text-align:center; color:#999; padding:20px;'>هیچ پرداختی ثبت نشده است</td></tr>";
          } else {
            var no = 0;
            var no1 = 1;
            rows_04.forEach((row) => {
...
app.post("/delete_stack_bill", function (req, res) {
  ...
  con.query(
    "select * from stack_bill_detail where id = '" + bill_id + "'",
    function (err4, rows_01) {
      var item_name = rows_01[0].item_name;
            ...
            function (err4, rows_09) {
              con.query(
                ...
                function (err4, rows_08) {
                  ...
                  var update_stck_fac_quna =
                    parseFloat(stak_fac_qun) + parseFloat(bill_qunatity);

                  con.query(
                    "update stack_factory_registration_list set quantity='" +
                      update_stck_fac_quna +
                      "' where item_name = '" +
                      item_name +
                      "' and item_type='" +
                      item_type +
                    ...
                    function (err2, rows_04) {
                      ...
                      con.query(
                        "delete from stack_bill_detail WHERE id = '" +
                          bill_id +
                          "'",
                        function (err, rows_02) {
                          if (err2) {
...
app.post("/update_show_edit1", function (req, res) {
  ...
  var loan_id = req.query.loan_id;
  con.query(
    "UPDATE `incoming_loan_list` SET `incoming_loan_id`='" +
      loan_id +
      "',`paid`='" +
      amount +
      "',`currency`='" +
...
app.post("/update_show_edit3", function (req, res) {
  ...
  con.query(
    "select * from raw_material_each_mahsol where stack_factory_id ='" +
      id_val +
      "'",
    function (err1, rows_08) {
      if (err1) {
        throw err1;
      } else {
...
app.post("/stuff_taken_amount", function (req, res) {
  ...
  var m = moment
    ...
    .format("YYYY/MM/DD");
  con.query(
    "INSERT INTO taken_amount(`stuff_id`, `amount`,`currency`,`ex_rate`,`date`) VALUES ('" +
      stf_id +
      "','" +
      amount +
      "','" +
...
app.post("/employ_salary_pay", function (req, res) {
  ...
  con.query(
    "select * from payable_amount where stuff_id = '" +
      stf_id +
      "' and date='" +
      m +
      "'",
    function (err, rows_01) {
      if (rows_01.length > 0) {
        ...
        con.query(
          "INSERT INTO payable_amount(`stuff_id`,`taken_amount`,`tax`,`overtime`,`payable`,`currency`,`ex_rate`,`date`) VALUES ('" +
            stf_id +
            "','" +
            payable +
            "','" +
...
app.post("/incoming_loan", function (req, res) {
  ...
  var m = moment
    ...
    .format("YYYY/MM/DD");
  con.query(
    "INSERT INTO incoming_loan(`borrower`,`lender`,`amount`,`installment_no`,`lender_contact`,`address`,`currency`,`ex_rate`,`benefit`,`date`) VALUES ('" +
      borrower +
      "','" +
      lender +
      "','" +
...
app.post("/outgoing_loan", function (req, res) {
  ...
  var m = moment
    ...
    .format("YYYY/MM/DD");
  con.query(
    "INSERT INTO outgoing_loan(`stuff_id`,`company_name`,`amount`,`installment_no`,`currency`,`ex_rate`,`tazkira`,`date`) VALUES ('" +
      stf_id +
      "','" +
      co_name +
      "','" +
...
app.post("/add_machinary", function (req, res) {
  ...
  var m = moment
    ...
    .format("YYYY/MM/DD");
  con.query(
    "INSERT INTO item_registration(`name`,`company_name`,`description`,`quantity`,`purchase_price`,`machine_life`,`currency`,`ex_rate`,`date`) VALUES ('" +
      name +
      "','" +
      co_name +
      "','" +
...
app.post("/add_tools", function (req, res) {
  ...
  var m = moment
    ...
    .format("YYYY/MM/DD");
  con.query(
    "INSERT INTO item_registration(`name`,`company_name`,`description`,`quantity`,`purchase_price`,`currency`,`ex_rate`,`date`) VALUES ('" +
      name +
      "','" +
      co_name +
      "','" +
...
app.post("/add_customer", function (req, res) {
  ...
  var m = moment
    ...
    .format("YYYY/MM/DD");
  con.query(
    "INSERT INTO customer_account(`name`,`last_name`,`company_name`,`contact`,`address`,`email`,`date`) VALUES ('" +
      name +
      "','" +
      last_name +
      "','" +
...
app.get("/emp_removal_reg.ejs", function (req, res) {
  con.query(
    "select round(sum(amount/ex_rate),3) as total_taken from taken_amount",
    function (err, rows_01) {
      var sqlquery =
        "SELECT taken_amount.id as tken_id,taken_amount.amount,taken_amount.currency,taken_amount.ex_rate,taken_amount.date as tkn_date , stuff_registration.* from taken_amount INNER JOIN stuff_registration ON taken_amount.stuff_id = stuff_registration.id";

      con.query(sqlquery, function (err, rows_04) {
        if (rows_04.length > 0) {
          var arr = [];
          for (var i = 0; i < rows_04.length; i++) {
            var sh = moment(rows_04[i].tkn_date, "YYYY/MM/DD")
              .locale("fa")
              .format("YYYY/MM/DD");
            arr += sh + ",";
          }
          var str_array = arr.split(",");
          res.render("emp_removal_reg", {
            data_04: rows_04,
            data_date: str_array,
            all_taken_amounts: rows_01[0].total_taken,
          });
        } else {
          res.send(
...
app.get("/emp_SP_reg.ejs", function (req, res) {
  con.query(
    "select round(sum(taken_amount/ex_rate),3) as total_paid from payable_amount",
    function (err, rows_01) {
      con.query(
        "SELECT stuff_registration.id,stuff_registration.name,stuff_registration.last_name,stuff_registration.salary,payable_amount.* FROM payable_amount INNER JOIN stuff_registration ON payable_amount.stuff_id=stuff_registration.id",
        function (err, rows_02) {
          if (rows_02.length > 0) {
            var arr = [];
            for (var i = 0; i < rows_02.length; i++) {
              var sh = moment(rows_02[i].date, "YYYY/MM/DD")
                .locale("fa")
                .format("YYYY/MM/DD");
              arr += sh + ",";
            }
            var str_array = arr.split(",");
            res.render("emp_SP_reg", {
              data_02: rows_02,
              data_date: str_array,
              all_stff_sal: rows_01[0].total_paid,
            });
          } else {
            res.send(
              "<h1 style='color:green; text-align:center;'>کارمند موجود نیست !</h1>"
...
app.post("/getUserName", function (req, res) {
  ...
  con.query(
    "select * from stack_factory_registration where item_name= '" + my_id + "'",
    function (err, rows_02) {
      console.log(
...
app.get("/emp-reg.ejs", function (req, res) {
  con.query(
    "select sum(salary) as total_salary from stuff_registration ",
    function (err, rows_01) {
      con.query("select * from stuff_registration ", function (err, rows_02) {
        if (rows_02.length > 0) {
          ...
          res.render("emp-reg", {
            data_02: rows_02,
            data_date: str_array,
            all_salary: rows_01[0].total_salary,
          });
        } else {
          res.send(
            "<h1 style='color:green; text-align:center;'>کارمند موجود نیست !</h1>"
...
app.get("/baraword.ejs", function (req, res) {
  con.query("select * from company_info ", function (err, rows_02) {
    con.query(
      "select item_name ,item_type,bill_no from stack_raw_materials group by item_name , item_type,bill_no  ",
      function (err, rows_03) {
        con.query(
...
app.get("/froshat_view.ejs", function (req, res) {
  con.query("SELECT * from customer_account", function (err, rows_03) {
    ...
    con.query(
      "SELECT customer_account.id as cust_id, customer_account.company_name, froshat_details.* FROM customer_account INNER JOIN froshat_details ON customer_account.id=froshat_details.cus_id ORDER BY froshat_details.date DESC, froshat_details.id DESC",
      function (err, rows_01) {
        if (err) {
...
app.get("/stack_view.ejs", function (req, res) {
  con.query(
    "SELECT stack_to_market_details.id, stack_to_market_details.bill_no,stack_to_market_details.date as stakc_date,stuff_registration.name,stuff_registration.contact,stuff_registration.email FROM stack_to_market_details INNER JOIN stuff_registration ON stack_to_market_details.stuff_id=stuff_registration.id",
    function (err, rows_01) {
      if (rows_01.length > 0) {
        var arr = [];
        for (var i = 0; i < rows_01.length; i++) {
          var sh = moment(rows_01[i].stakc_date, "YYYY/MM/DD")
            .locale("fa")
            .format("YYYY/MM/DD");
          arr += sh + ",";
        }
...
app.get("/city_exsistance.ejs", function (req, res) {
  con.query(
    "SELECT * FROM `stack_to_market_lists`order BY sell_price ASC",
    function (err, rows_01) {
      if (rows_01.length > 0) {
...
app.post("/searching_bill_no_01", function (req, res) {
  ...
  con.query(
    "SELECT customer_account.company_name, froshat_details.* FROM customer_account INNER JOIN froshat_details ON customer_account.id=froshat_details.cus_id WHERE  bill_no ='" +
      my_id +
      "' ",
    function (err, rows_02) {
      if (rows_02.length > 0) {
        ...
        con.query(
          "select * from froshat_details where bill_no = '" + my_id + "'",
          function (err, rows0002) {
...
app.post("/sending_stock_details", function (req, res) {
  ...
  con.query(
    "select * from stack_bill_detail WHERE stack_to_m_detail_id = '" +
      my_id +
      "'",
    function (err, rows_02) {
      var table_data = "";
      ...
      rows_02.forEach((row) => {
        ...
        table_data +=
          ...
          row.id +
          ")" +
          " data-toggle='modal' data-target='#basicModal' href=# style='color:green;'>ویرایش</a></td>";
        table_data += "</tr>";
        no++;
      });
      res.send(table_data);
...
app.post("/sending_all_price_details", function (req, res) {
  ...
  con.query(
    "SELECT * FROM `mahsol_price` where set_p_id = '" + my_id + "'",
    function (err, rows_02) {
      var table_data = "";
      var no = 1;
      rows_02.forEach((row) => {
...
app.get("/hb-reg.ejs", function (req, res) {
  con.query(
    "SELECT SUM(amount) as tken_amount from partner_taken_amount ",
    function (err, rows_03) {
      con.query(
        "SELECT partner_taken_amount.id,partner_taken_amount.amount,partner_taken_amount.document,partner_taken_amount.date,partner_registration.full_name,partner_registration.location,partner_registration.contact,partner_registration.email,partner_registration.address,partner_registration.participant_percentage FROM partner_taken_amount INNER JOIN partner_registration ON partner_taken_amount.partner_id = partner_registration.id ",
        function (err, rows_02) {
          if (rows_02.length > 0) {
            var arr = [];
            for (var i = 0; i < rows_02.length; i++) {
            ...
            res.render("hb-reg", {
              data_02: rows_02,
              date_data: str_array,
              total_rwos: rows_03[0].tken_amount,
            });
          } else {
            res.send(
...
app.get("/holder_billance.ejs", function (req, res) {
  con.query(
    "select COALESCE(sum(total_amount), 0)as income from froshat_details ",
    function (err, rows_01) {
      if (rows_01[0].income.length < 0) {
        var incom = 0;
      } else {
        con.query(
          "SELECT *,(SELECT COALESCE(SUM(amount), 0)AS total FROM partner_taken_amount WHERE partner_id=partner_registration.id) AS totalcount FROM partner_registration ",
          function (err, rows_02) {
            if (rows_02.length > 0) {
              con.query(
                "select sum(participant_percentage) as all_assets from partner_registration ",
                function (err, rows_03) {
                  ...
                  for (var j = 0; j < rows_02.length; j++) {
                    var each_asset =
                      parseFloat(rows_02[j].participant_percentage * 100) /
                      partners_asset;
                  ...
                  for (var k = 0; k < cal_each_income_arr.length; k++) {
                    var each_income =
                      (parseFloat(cal_each_income_arr[k]) * parseFloat(incom)) /
                      100;
                    cal_each_partner_income += roundTo(each_income, 2) + ",";
                  }
                  var cal_each_partner_income_arr =
                    cal_each_partner_income.split(",");
                  console.log(cal_each_partner_income_arr);

                  res.render("holder_billance", {
                    partner_details: rows_02,
...
app.get("/iloan_reg.ejs", function (req, res) {
  con.query(
    "SELECT sum((amount+benefit))AS total FROM incoming_loan WHERE currency='دالر'",
    function (err, rows_05) {
      con.query(
        "SELECT sum((amount+benefit))AS total FROM incoming_loan WHERE currency='افغانی'",
        function (err, rows_06) {
          con.query(
            "SELECT *,(SELECT COALESCE(SUM(paid), 0)AS total FROM incoming_loan_list WHERE incoming_loan_id =incoming_loan.id) AS totalcount FROM incoming_loan ",
            function (err, rows_02) {
              if (rows_02.length > 0) {
                var arr = [];
                for (var i = 0; i < rows_02.length; i++) {
                ...
                res.render("iloan_reg", {
                  data_02: rows_02,
                  data_date: str_array,
                  all_in_dollor: rows_05[0].total,
                  all_in_afghani: rows_06[0].total,
                });
              } else {
                res.send(
...
app.get("/mt_reg.ejs", function (req, res) {
  con.query(
    "SELECT sum(purchase_price)AS price FROM item_registration where machine_life IS NOT NULL",
    function (err, rows_05) {
      con.query(
        "SELECT * FROM  item_registration where machine_life IS NOT NULL",
        function (err, rows_03) {
          if (rows_03.length > 0) {
            var arr = [];
            for (var i = 0; i < rows_03.length; i++) {
            ...
            res.render("mt_reg", {
              data_03: rows_03,
              date_data: str_array,
              price: rows_05[0].price,
              all_years: some_err_years,
              all_days: some_err,
            });
          } else {
            res.send(
...
app.get("/oloan_reg.ejs", function (req, res) {
  con.query(
    "SELECT round(sum(amount/ex_rate),3)AS total FROM outgoing_loan",
    function (err, rows_05) {
      con.query(
        "SELECT outgoing_loan.stuff_id as stf_id, stuff_registration.name FROM outgoing_loan INNER JOIN stuff_registration ON outgoing_loan.stuff_id=stuff_registration.id",
        function (err, rows_04) {
          if (rows_04.length > 0) {
            ...
            con.query(
              "SELECT outgoing_loan.id,outgoing_loan.stuff_id as stf_id,outgoing_loan.company_name,outgoing_loan.amount,outgoing_loan.installment_no,outgoing_loan.currency,outgoing_loan.ex_rate,outgoing_loan.tazkira,outgoing_loan.date,(SELECT COALESCE(SUM(paid), 0)AS total FROM outgoing_loan_list WHERE outgoing_loan_id =outgoing_loan.id) AS totalcount FROM outgoing_loan ",
              function (err, rows_03) {
                var arr = [];
                for (var i = 0; i < rows_03.length; i++) {
                  var sh = moment(rows_03[i].date, "YYYY/MM/DD")
...
app.post("/add_expense", function (req, res) {
  ...
  var m = moment
    ...
    .format("YYYY/MM/DD");
  con.query(
    "INSERT INTO expenses(`category_id`, `quantity`,`price`,`description`,`ex_rate`,`currency`,`date`) VALUES ('" +
      dis_type +
      "','" +
      amount +
      "','" +
...
app.post("/add_expense1", function (req, res) {
  ...
  con.query(
    "INSERT INTO expense_category(`name`) VALUES ('" + category_type + "')",
    function (err, rows) {
      if (err) {
        throw err;
      } else {
...
app.get("/raw_material_store.ejs", function (req, res) {
  con.query(
    "SELECT round(sum((price * quantity)),3)AS total FROM stack_raw_materials WHERE currency='دالر'",
    function (err, rows_05) {
      con.query(
        "SELECT round(sum((price * quantity)),3)AS total FROM stack_raw_materials WHERE currency='افغانی'",
        function (err, rows_06) {
          con.query(
            "select * from stack_raw_materials",
            function (err, rows_04) {
              if (rows_04.length > 0) {
                var arr = [];
                for (var i = 0; i < rows_04.length; i++) {
                ...
                res.render("raw_material_store", {
                  data_04: rows_04,
                  date_data: str_array,
                  dollor_price: rows_05[0].total,
                  afghani_price: rows_06[0].total,
                });
              } else {
                res.send(
...
app.get("/raw_materials.ejs", function (req, res) {
  con.query("select * from material_type", function (err, rows_04) {
    con.query(
      "SELECT MAX(id) as new_bill  FROM material_type",
      function (err, rows_03) {
        res.render("raw_materials", {
          data_04: rows_04,
...
app.post("/stack_raw_regist", function (req, res) {
  ...
  con.query(
    "INSERT INTO stack_raw_materials(`buy_type`,`company_name`,`contact`,`bill_no`,`item_name`,`item_type`,`buy_place`,`description`,`quantity`,`price`,`currency`,`ex_rate`,`date`) VALUES ('" +
      item_type +
      "','" +
      company_name +
      "','" +
...
app.post("/add_new_types", function (req, res) {
  ...
  con.query(
    "select * from material_type where serial_number ='" + serail_no + "'",
    function (err, rows0002) {
      if (rows0002.length > 0) {
        res.send("موجود است ");
      } else {
        con.query(
          "INSERT INTO `material_type`( `type`, `serial_number`) VALUES ('" +
            type +
            "','" +
            serail_no +
            "')",
...
app.post("/edit_add_type_02", function (req, res) {
  ...
  var ser_no = req.query.ser_no;
  con.query(
    "update ready_materials_type set name='" +
      mahsol_name +
      "', type='" +
      type +
      "',serial_no='" +
          ...
          for (var i = 0; i < rows_0002.length; i++) {
            (function (i) {
              setTimeout(function () {
                ...
                con.query(
                  "insert into raw_material_each_mahsol (stack_factory_id,item_name,item_type,quantity)values('" +
                    stack_fac_reg +
                    "','" +
                    mahsol_item_name +
                    "','" +
          ...
          con.query(
            ...
            function (err, rows_02) {
              if (rows_02.length > 0) {
                ...
                var update_qunatity =
                  parseFloat(db_quantity) + parseFloat(web_quantity);
                con.query(
                  "update stack_factory_registration_list set quantity = '" +
                    update_qunatity +
                    "' where item_name='" +
...
app.post("/market_reg", function (req, res) {
  ...
  var m = moment
    .from(date1, "fa", "YYYY/MM/DD")
    .locale("en")
    .format("YYYY/MM/DD"); //conver farsi date to gregorian
  ...
  con.query(
    ...
    function (err, rows_07) {
      ...
      if (rows_07.length > 0) {
        con.query(
          "select quantity from stack_to_market where item_name='" +
            name +
            "' and item_type='" +
            item_type +
            "'",
          function (err, rows_032) {
                ...
                if (err) {
                  ...
                  con.query(
                    ...
                    function (err, rows_02) {
                      ...
                      con.query(
                        "update stack_factory_registration set quantity='" +
                          update_qunt +
                          "' where item_name='" +
                          name +
                          "' and item_type='" +
        ...
        con.query(
          "INSERT INTO stack_to_market(`item_name`,`item_type`,`fixed_price`,`sell_price`,`quantity`,`date`) VALUES ('" +
            name +
            "','" +
            item_type +
            "','" +
...
app.post("/up_image", function (req, res) {
  if (!(req.files && req.files.upload)) {
    ...
    con.query(
      "INSERT INTO stuff_registration(`name`,`last_name`,`contact`,`email`,`tazkira`,`address`,`job_place`,`job_type`,`id_number`,`salary`,`image`,`date`) VALUES ('" +
        name2 +
        "','" +
        last_name +
        "','" +
...
app.post("/partner_percent", function (req, res) {
  if (!(req.files && req.files.doc_scan)) {
    ...
    con.query(
      "INSERT INTO partner_registration(`full_name`,`location`,`contact`,`email`,`address`,`participant_percentage`,`document`,`date`) VALUES ('" +
        name2 +
        "','" +
        location +
        "','" +
...
app.post("/partner_bardasht", function (req, res) {
  ...
  var file_name = file.name;
  con.query(
    "INSERT INTO partner_taken_amount(`partner_id`,`amount`,`document`,`date`) VALUES ('" +
      partner_id +
      "','" +
      taken_amount +
      "','" +
...
app.post("/add_company_info", function (req, res) {
  ...
  var m = moment
    .from(agreement_date, "fa", "YYYY/MM/DD")
    .locale("en")
    .format("YYYY/MM/DD");
  var m1 = moment
    .from(date, "fa", "YYYY/MM/DD")
...
app.get("/tools-rge.ejs", function (req, res) {
  con.query(
    "SELECT sum(purchase_price)AS price FROM item_registration where machine_life IS NULL",
    function (err, rows_05) {
      con.query(
        "select * from item_registration where machine_life IS  NULL",
        function (err, rows_04) {
          if (rows_04.length > 0) {
            var arr = [];
            for (var i = 0; i < rows_04.length; i++) {
            ...
            res.render("tools-rge", {
              data_04: rows_04,
              data_date: str_array,
              purchase_price: rows_05[0].price,
            });
          } else {
            res.send(
              "<h1 style='color:green; text-align:center;'>جنس در گدام موجود نیست !</h1>"
        ...
        function (err, rows_01) {
          con.query(
            ...
            function (err, rows_02) {
              ...
              for (var i = 0; i < rows_02.length; i++) {
                var each_asset =
                  parseFloat(rows_02[i].participant_percentage * 100) /
                  partners_asset;
...
app.get("/rmf_reg.ejs", function (req, res) {
  con.query(
    "SELECT round(sum((fixed_price * quantity)/ex_rate),3)AS total FROM stack_factory_registration",
    function (err, rows_05) {
      con.query(
        "SELECT * FROM `stack_factory_registration` ORDER BY serial_number ASC ",
        function (err, rows_04) {
          if (rows_04.length > 0) {
            var arr = [];
            for (var i = 0; i < rows_04.length; i++) {
  ...
  con.query(
    ...
    function (err, rows_04) {
      con.query(
        ...
        function (err, rows_03) {
          con.query(
            " SELECT round(SUM(total_amount/ex_rate),3)AS benefit FROM froshat_details",
            function (err, rows_05) {
              con.query(
                "SELECT round(SUM(amount/ex_rate),3)AS company_loan FROM incoming_loan",
                function (err, rows_02) {
                  con.query(
                    "SELECT round(SUM(amount/ex_rate),3)AS stuff_loan FROM outgoing_loan",
                    function (err, rows_01) {
                      con.query(
                        "SELECT round(SUM((quantity * price)/ex_rate),3)AS sell_total4 FROM stack_raw_materials",
                        function (err, rows_06) {
                          res.render("reports", {
                            expenses: rows_04[0].total,
                            sell_amount: rows_03[0].sell_total,
                            comp_loan: rows_02[0].company_loan,
                            stuff_loan: rows_01[0].stuff_loan,
...
app.post("/from_date_to_date", function (req, res) {
  ...
  con.query(
    "SELECT COALESCE(sum(quantity*price),0) as expse FROM `expenses` WHERE date BETWEEN '" +
      f_d +
      "' AND '" +
      t_d +
      "'",
    function (err, rows_01) {
      con.query(
        "SELECT COALESCE(sum(total_amount),0) as tot_price FROM froshat_details WHERE date BETWEEN '" +
          f_d +
          "' AND '" +
          t_d +
          "'",
        function (err, rows_02) {
          con.query(
            "SELECT COALESCE(sum(amount),0) as tot_loan FROM incoming_loan WHERE date BETWEEN '" +
              f_d +
              "' AND '" +
              t_d +
              "'",
            function (err, rows_03) {
              con.query(
                "SELECT COALESCE(sum(amount), 0) as tot_outloan FROM outgoing_loan WHERE date BETWEEN '" +
                  f_d +
                  "' AND '" +
                  t_d +
                  "'",
                function (err, rows_04) {
                  con.query(
                    "SELECT round(SUM((quantity* price)/ex_rate),3) as tot_raw_purchase FROM stack_raw_materials WHERE date BETWEEN '" +
                      f_d +
                      "' AND '" +
                      t_d +
                      "'",
...
app.post("/from_date_to_expense_01", function (req, res) {
  ...
  con.query(
    "SELECT expenses.*, expense_category.name FROM expenses INNER JOIN expense_category ON expenses.category_id = expense_category.id WHERE date BETWEEN '" +
      f_d +
      "' AND '" +
      t_d +
      "'",
      ...
      if (err) {
        ...
        rows_04.forEach((row) => {
          ...
          table_data +=
            ...
            row.id +
            ")" +
            " href='#' style='color:red;'>  </a>     <a onclick=" +
            "cat_edit(" +
            row.id +
            ")" +
            " href=# style='color:green;'></a></td>";
          table_data += "</tr>";
          no++;
        });
        res.send(table_data);
...
app.post("/from_date_to_raw_m_01", function (req, res) {
  ...
  con.query(
    "SELECT * from stack_raw_materials WHERE date BETWEEN '" +
      f_d +
      "' AND '" +
      t_d +
      "'",
...
app.post("/from_date_to_rmf_reg_01", function (req, res) {
  ...
  con.query(
    "SELECT * from stack_factory_registration WHERE date BETWEEN '" +
      f_d +
      "' AND '" +
      t_d +
      "'",
      ...
      if (rows_04.length > 0) {
        if (err) {
          ...
          rows_04.forEach((row) => {
            ...
            table_data += "<td>" + str_array[no] + "</td>";
            table_data +=
              "<td><button data-toggle='tooltip' title='حذف' class='edt del' onclick=" +
              "delete1(" +
              row.id +
              ")" +
              "><img width='15px' src='assets/img/last-project/delete.svg' alt=''>  </button>     <button title='ویرایش' data-toggle='tooltip' onclick=" +
              "update_rmf(" +
              row.id +
              ")" +
              "  class='edt edit-tbl'><img width='15px' src='assets/img/last-project/edit.svg' alt=''></button></td>";
            table_data += "</tr>";
            no++;
          });
          res.send(table_data);
...
app.post("/from_date_to_city_m_01", function (req, res) {
  ...
  con.query(
    "SELECT * from stack_to_market WHERE date BETWEEN '" +
      f_d +
      "' AND '" +
      t_d +
      "'",
      ...
      if (err) {
        ...
        if (rows_04.length > 0) {
          ...
          rows_04.forEach((row) => {
            ...
            table_data +=
              "<td>" +
              parseFloat(row.sell_price) * parseFloat(row.quantity) +
              "</td>";
            /* table_data += "<td>"+ row.quantity+ "</td>"; */
            // table_data += "<td>"+ parseFloat(row.price) *  parseFloat(row.quantity)+ "</td>";
            table_data += "<td>" + str_array[no] + "</td>";
...
app.post("/from_date_to_emp_m_01", function (req, res) {
  ...
  con.query(
    "SELECT * from stuff_registration WHERE date BETWEEN '" +
      f_d +
      "' AND '" +
      t_d +
      "'",
        ...
        if (rows_04.length > 0) {
          ...
          rows_04.forEach((row) => {
            ...
            table_data +=
              ...
              row.id +
              ")" +
              " href='#'' class='edt del'><img width='15px' src='assets/img/last-project/delete.svg' alt=''> </a>     <a onclick=" +
              "update_employees(" +
              row.id +
              ")" +
              " data-toggle='tooltip' href=# class='edt edit-tbl'  ><img width='15px' src='assets/img/last-project/edit.svg' alt=''></a></td>";
            table_data += "</tr>";
            no++;
          });
          res.send(table_data);
...
app.post("/from_date_to_emp_removable_01", function (req, res) {
  ...
  con.query(
    "SELECT taken_amount.id as tken_id,taken_amount.amount,taken_amount.currency,taken_amount.ex_rate,taken_amount.date as tkn_date , stuff_registration.* from taken_amount INNER JOIN stuff_registration ON taken_amount.stuff_id = stuff_registration.id WHERE taken_amount.date BETWEEN '" +
      f_d +
      "' AND '" +
      t_d +
      "'",
...
app.post("/from_date_to_emp_salpay_01", function (req, res) {
  ...
  con.query(
    "SELECT stuff_registration.id,stuff_registration.name,stuff_registration.last_name,stuff_registration.salary,payable_amount.* FROM payable_amount INNER JOIN stuff_registration ON payable_amount.stuff_id=stuff_registration.id WHERE payable_amount.date BETWEEN '" +
      f_d +
      "' AND '" +
      t_d +
      "'",
        ...
        if (rows_04.length > 0) {
          ...
          rows_04.forEach((row) => {
            ...
            table_data +=
              ...
              row.id +
              ")" +
              " data-toggle='tooltip' class='edt edit-tbl'  ><img width='15px' src='assets/img/last-project/edit.svg' alt=''></a></td>";
            table_data += "</tr>";
            no++;
          });
          res.send(table_data);
      ...
      if (err) {
        ...
        if (rows_04.length > 0) {
          ...
          rows_04.forEach((row) => {
            ...
            table_data +=
              "<td>" + parseFloat(row.amount - row.totalcount) + "</td>";
...
app.post("/from_date_to_iloan_reg_01", function (req, res) {
  ...
  con.query(
    "SELECT *,(SELECT COALESCE(SUM(paid), 0)AS total FROM incoming_loan_list WHERE incoming_loan_id =incoming_loan.id) AS totalcount FROM incoming_loan WHERE date BETWEEN '" +
      f_d +
      "' AND '" +
      t_d +
      "'",
      ...
      if (err) {
        ...
        if (rows_04.length > 0) {
          ...
          rows_04.forEach((row) => {
            ...
            table_data +=
              "<td>" + parseFloat(row.amount + row.benefit) + "</td>";
            table_data += "<td>" + row.totalcount + "</td>";
            table_data +=
              "<td>" +
              parseFloat(row.amount + row.benefit - row.totalcount) +
              "</td>";
            table_data += "<td>" + row.address + "</td>";
...
app.post("/from_date_to_tools_reg_01", function (req, res) {
  ...
  con.query(
    "select * from item_registration where machine_life IS  NULL and date BETWEEN '" +
      f_d +
      "' AND '" +
      t_d +
      "'",
...
app.post("/from_date_to_tools_reg_02", function (req, res) {
  ...
  con.query(
    "SELECT * FROM  item_registration where machine_life IS NOT NULL and date BETWEEN '" +
      f_d +
      "' AND '" +
      t_d +
      "'",
...
app.post("/from_date_to_cs_reg_01", function (req, res) {
  ...
  con.query(
    "SELECT *,(SELECT round(COALESCE(SUM(total_amount /ex_rate),0),3)AS total FROM froshat_details WHERE cus_id=customer_account.id) AS total_amount ,(SELECT round(COALESCE(SUM(paid_amount/ex_rate),0),3)AS total_paid FROM froshat_details WHERE cus_id=customer_account.id) AS cus_paid FROM customer_account where  date BETWEEN '" +
      f_d +
      "' AND '" +
      t_d +
      "'",
      ...
      if (err) {
        ...
        if (rows_04.length > 0) {
          ...
          rows_04.forEach((row) => {
            ...
            table_data +=
              "<td>" + parseFloat(row.total_amount - row.cus_paid) + "</td>";
